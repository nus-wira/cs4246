...
def value_iteration(trn_fn: np.ndarray, rwd_fn: np.ndarray, gamma: float):
    '''
    ...
    while should_iterate:
        ...
        # For each state
        for i in range(S):
            # Get utilities for taking each action
            next_state_utils = rwd_fn[i, :].T + gamma * value_fn
            # Factor in transition probabilities
            action_utils = matmul(trn_fn[:, i, :], next_state_utils)

            # Get best action, and utility
            best_act = np.argmax(action_utils)
            best_util = action_utils[best_act]

            # Should iterate if diff > delta
            diff = best_util - value_fn[i]
            should_iterate |= diff > delta

            # Update value_fn and policy
            value_fn[i] = best_util
            policy[i] = best_act
