...
class GeneratePDDL_Stationary :
    ...
    def get_grid_cell(self, x, y):
        return self.grid_cell_list[x * self.num_lanes + y]
    ...
    def generateGoalString(self) :
        ...
        return f"(at {self.grid_cell_list[goal_pos.y * self.num_lanes + goal_pos.x]} agent1)"
...
def generateDomainPDDLFile(gen):
    ...
    gen.addAction(
        name="FORWARD", 
        parameters = (("agent", "agent"), ("p1", "gridcell"), ("p2", "gridcell")), 
        precondition_string = "(and (at ?p1 ?agent) (forward_next ?p1 ?p2) (not (blocked ?p2)))", 
        effect_string="(and (not (at ?p1 ?agent)) (at ?p2 ?agent) (not (blocked ?p1)) (blocked ?p2))"
    )
