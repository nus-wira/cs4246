...
class GeneratePDDL_Stationary :
    ...
    def generateGoalString(self) :
        ...
        '''    
        goal_pos = self.state.finish_position
        grid_cells = [self.get_grid_cell(goal_pos.x, goal_pos.y, t) for t in range(self.width)]
        return "(or " + ' '.join([f"(at {grid_cell} agent1)" for grid_cell in grid_cells]) + ")"
...
def generateDomainPDDLFile(gen : GeneratePDDL_Stationary):
    '''
    ...
    (forwardX_next ?pt1 ?pt2) : pt2 is the next location + time of the car when it takes the FORWARDX action from pt1
...
def generateProblemPDDLFile(gen: GeneratePDDL_Stationary):
    '''
    Function that specifies the domain and generates the PDDL Domain File.
